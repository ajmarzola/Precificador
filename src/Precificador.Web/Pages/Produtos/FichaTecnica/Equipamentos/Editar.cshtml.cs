...
public sealed class EditarModel(PrecificadorDbContext context) : PageModel
{
    ...
    { var produtoId = Id("produtoId"); var usoId = Id("usoId"); if (!await CarregarAsync(produtoId, usoId, true)) return NotFound(); var usoAtual = usoRastreado!; var potenciaValida = UsoEquipamentoFichaFormulario.TentarObterPotencia(ModelState, Input.PotenciaKw, out var potencia); if (Input.TempoUsoMinutos is null || Input.TempoUsoMinutos <= 0) ModelState.AddModelError("Input.TempoUsoMinutos", "O tempo de uso deve ser maior que zero."); if (!potenciaValida || !ModelState.IsValid) return Page(); var tempoUsoMinutos = Input.TempoUsoMinutos; if (tempoUsoMinutos is null) return Page(); var normalizado = Normalizar(Input.NomeEquipamento); if (await context.UsosEquipamentosFicha.AnyAsync(u => u.FichaTecnicaId == usoAtual.FichaTecnicaId && u.Id != usoAtual.Id && u.NomeEquipamentoNormalizado == normalizado.ToUpperInvariant())) { ModelState.AddModelError("Input.NomeEquipamento", "Este equipamento já foi adicionado à ficha técnica."); return Page(); } try { usoAtual.AtualizarDados(Input.NomeEquipamento!, potencia, tempoUsoMinutos.Value); } catch (ArgumentException e) { ModelState.AddModelError("Input.NomeEquipamento", e.Message); return Page(); } await context.SaveChangesAsync(); TempData["MensagemSucesso"] = "Equipamento atualizado com sucesso."; return RedirectToPage("/Produtos/FichaTecnica", new { id = produtoId }); }
