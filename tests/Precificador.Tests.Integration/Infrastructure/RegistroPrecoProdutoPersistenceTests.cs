...
{
    [Fact]
    public async Task P1_P2_P4_P8_Migration_e_round_trip_preservam_historico_append_only_sem_backfill()
    {
        await using var connection = await AbrirAsync();
        await using var context = Criar(connection, 1);
        await context.Database.MigrateAsync("20260915154603_AddPercentualPerdaItemFichaTecnica");
    ...
    {
        await using var connection = await AbrirAsync();
        await using var empresaUm = Criar(connection, 1); await empresaUm.Database.MigrateAsync();
        ...
        await using var empresaDois = Criar(connection, 2); var produtoDois = Produto.Criar(2, "Dois", .3m); empresaDois.Produtos.Add(produtoDois); await empresaDois.SaveChangesAsync();
    ...
    private static async Task<SqliteConnection> AbrirAsync() { var c = new SqliteConnection("Data Source=:memory:"); await c.OpenAsync(); return c; }
    private static PrecificadorDbContext Criar(SqliteConnection c, int empresa) => new(new DbContextOptionsBuilder<PrecificadorDbContext>().UseSqlite(c).Options, new Contexto(empresa));
