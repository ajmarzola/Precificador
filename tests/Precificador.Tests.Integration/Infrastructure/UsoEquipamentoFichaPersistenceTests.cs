...
{
    [Fact]
    public async Task P1_P2_P3_MigrationCriaTabelaEIndiceUnico()
    {
        var connectionString = await SqlServerTestDatabase.CriarConnectionStringAsync("UsoEquipamentoFicha");
        await using var context = CriarContexto(connectionString, 1); await context.Database.MigrateAsync();
    ...
    {
        var connectionString = await SqlServerTestDatabase.CriarConnectionStringAsync("UsoEquipamentoFicha");
        await using (var um = CriarContexto(connectionString, 1)) { await um.Database.MigrateAsync(); um.Empresas.Add(Empresa.Criar("Dois")); await um.SaveChangesAsync(); }
        int fichaDois; await using (var dois = CriarContexto(connectionString, 2)) { fichaDois = await CriarFichaAsync(dois, 2); dois.UsosEquipamentosFicha.Add(UsoEquipamentoFicha.Criar(2, fichaDois, "Forno", 1m, 10)); await dois.SaveChangesAsync(); }
        await using var contextoUm = CriarContexto(connectionString, 1); var fichaUm = await CriarFichaAsync(contextoUm, 1); Assert.Empty(await contextoUm.UsosEquipamentosFicha.ToListAsync());
