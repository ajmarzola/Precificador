...
public sealed class RegistroPrecoProdutoPageTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task W1_W2_W3_W4_W5_W12_W13_W21_W22_W23_Get_e_post_respeitam_contrato_comercial()
    {
        ...
        get.EnsureSuccessStatusCode(); Assert.Contains("Preço de prateleira", html); Assert.Contains("Custo unitário", html); Assert.Contains("Preço teórico", html); Assert.Contains("Preço sugerido", html); Assert.Contains("Inativo", html); Assert.Contains("R$ 11,00", html); Assert.Contains("R$ 15,71", html);
        ...
        var registro = Assert.Single(await RegistrosAsync(produto, 1)); Assert.Equal(1m, registro.PrecoPrateleira); Assert.Equal(11m, registro.CustoReferencia); Assert.Equal(.3m, registro.MargemReferencia); Assert.Equal(15.72m, registro.PrecoSugerido); Assert.Equal(.1m, registro.ReservaComercialReferencia); Assert.NotEqual(default, registro.DataReferencia); Assert.False((await ProdutoAsync(produto, 1)).Ativo);
    ...
    public async Task W14_W15_W16_W17_W18_W19_Snapshot_usa_estado_vigente_no_post_e_preserva_anterior()
    {
        ...
        var primeiro = Assert.Single(await teste.RegistrosAsync(produto, 1)); Assert.Equal(new DateOnly(2030, 1, 2), primeiro.DataReferencia); Assert.Equal(22m, primeiro.CustoReferencia); Assert.Equal(.5m, primeiro.MargemReferencia); Assert.Equal(44m, primeiro.PrecoSugerido); Assert.Equal(.2m, primeiro.ReservaComercialReferencia);
    ...
    private async Task<int> CriarProdutoPrecificavelAsync(int empresa, bool ativo)
    {
        ...
        var configuracao = await context.ConfiguracoesPrecificacaoEmpresas.SingleAsync(); configuracao.Atualizar(.1m, null, null, .01m, .1m);
        ...
        var ficha = FichaTecnica.Criar(empresa, produto.Id, 1m); context.FichasTecnicas.Add(ficha); var insumo = Insumo.Criar(empresa, Guid.NewGuid().ToString(), CategoriaInsumo.MateriaPrima, UnidadeMedida.Unidade); context.Insumos.Add(insumo); await context.SaveChangesAsync();
