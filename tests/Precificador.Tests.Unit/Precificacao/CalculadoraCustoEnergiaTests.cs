...
public sealed class CalculadoraCustoEnergiaTests
{
    ...
    [Fact] public void Calcular_MultiplosUsos_SomaTotal() { var r = CalculadoraCustoEnergia.Calcular(2m, [new(1, 1m, 30), new(2, 3m, 30)]); Assert.Equal(4m, r.CustoEnergiaLote); }
    ...
    [Fact] public void Calcular_EntradaInvalida_Rejeita() { Assert.Throws<ArgumentOutOfRangeException>(() => CalculadoraCustoEnergia.Calcular(1m, [new(1, 0m, 1)])); Assert.Throws<ArgumentOutOfRangeException>(() => CalculadoraCustoEnergia.Calcular(1m, [new(1, 1m, 0)])); Assert.Throws<ArgumentOutOfRangeException>(() => CalculadoraCustoEnergia.Calcular(-1m, [])); }
}
